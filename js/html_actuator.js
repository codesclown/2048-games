function HTMLActuator() {
  ...
  if (!this.messageContainer) {
    console.log(
      "Game message container not found - will use ultimate button fix for game over",
    );
  }
...
HTMLActuator.prototype.actuate = function (grid, metadata) {
      ...
      if (tile) {
        ...
        var tileClone = {
          ...
          previousPosition: tile.previousPosition
            ? { x: tile.previousPosition.x, y: tile.previousPosition.y }
            : null,
          mergedFrom: null,
        };
  ...
  window.requestAnimationFrame(function () {
    ...
    // MOVED TO TOP: Ensure CSS is ready *before* we render tiles to prevent flickering
    if (grid.size > 4 && self.currentGridSize !== grid.size) {
      self.injectGridCSS(grid.size);
      self.currentGridSize = grid.size;
    }
